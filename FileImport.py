...
def send_message(telephone, webdriver, message):
    try:
        search_text_box_and_send(webdriver, '//div[@contenteditable="true"][@data-tab="3"]', telephone)
        search_text_box_and_send(webdriver, '//div[@contenteditable="true"][@title="Type a message"]', message)
...
for index, row in df.iterrows():
    tel_value = row['TEL']
    name_value = row['NAME']
    message = base_message.replace(base_message, tel_value, name_value)
    if not math.isnan(tel_value):
        tel_value = int(tel_value)
        send_message(tel_value, webdriver, message)